...
> The current `cluster-api-cce-controller:latest` digest is `bc2d39e1` (includes kubeconfig endpoint override + 429 back-off fix + cluster/node-pool `additionalTags` + the delete/backoff and tag-ownership audit fixes).
