...
2. `ResolveIdentity`：按 identityRef Kind——
   ...
   - `CCEClusterStaticIdentity` → 校验 allowedNamespaces → 读 `cce-provider-system` 命名空间下 Secret（**硬编码命名空间**）。
...
- CAPA StaticIdentity Secret 命名空间取自 `system.GetManagerNamespace()`；CCE 硬编码 `cce-provider-system`。
...
2. **StaticIdentity Secret 命名空间硬编码** `cce-provider-system`（§4）：应取 manager namespace（对齐 CAPA `system.GetManagerNamespace()`），否则部署到非默认命名空间时失效。
