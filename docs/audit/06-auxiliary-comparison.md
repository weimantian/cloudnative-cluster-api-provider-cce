...
- `ResolveIdentity(ctx, client, namespace, identityRef)`：三种身份 ——
  ...
  - `CCEClusterStaticIdentity` → 从 **硬编码 namespace `cce-provider-system`** 读 Secret（`accessKey`/`secretKey`）。
...
| Static identity Secret namespace | 经 identityRef 指定 | 🟡 **硬编码 `cce-provider-system`** |
...
- `internal/scope/scope.go` 将 static identity Secret 固定读 `cce-provider-system` namespace，CAPA 不硬编码。跨 namespace 部署 static identity 会受限。
