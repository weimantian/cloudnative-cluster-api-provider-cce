...
> 当前 `cluster-api-cce-controller:latest` digest `073d06b5`（含 kubeconfig 端点覆盖 + 429 退避修复 + 集群/节点池 `additionalTags` + 删除/退避与标签所有权审计修复 + 第 2 轮质量审计全量整改（R2-1…R2-15）与旁支项收口，并含公网 EIP 自动绑定（endpointAccess.public））。
