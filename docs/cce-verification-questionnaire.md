...
| Q11 | 集群升级工作流 | ✅ 仅原地升级;**官方路径表确认 v1.34→v1.35 支持(cce_10_0197)**;升级 API 可用(实测);三次实测(Standard/Turbo/带节点池)当前 targets=[]——官方路径=产品支持,API 目标=动态开放,controller 按空目标正常处理 | 升级耗时量级(平台开放目标后实测,测试已就绪);需咨询华为云开放时间 | 升级 API + 冒烟 + cce_10_0197 |
