...
| Q11 | 集群升级工作流 | ✅ **完整确认(官方 cce_10_0197,2026-07-28 版)**:路径表(v1.34→v1.35 等)、维护周期 24 个月、批次 4 的幂、备份回滚、证书/OIDC;升级 API 可用(实测);**v1.33→v1.34 目标开放并跑通工作流(3 真实约束已修复);v1.34 目标 v1.35 未开放——目标按版本线动态开放,controller 按空目标正常处理** | 完整成功升级耗时(预检查通过后实测) | 升级 API + 冒烟 + cce_10_0197 |
