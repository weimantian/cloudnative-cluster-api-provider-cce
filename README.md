...
flowchart TB
    subgraph OPS["Ops / Delivery layer"]
        ...
        GH["GitHub Release v0.1.1<br/>clusterctl.yaml · components · cluster-templates"]
...
- **`clusterctl` compatible** — `metadata.yaml` + `infrastructure-components.yaml` published as [GitHub Release v0.1.1](https://github.com/weimantian/cloudnative-cluster-api-provider-cce/releases/tag/v0.1.1); `clusterctl describe cluster` / `get kubeconfig` support.
...
4. **Configure clusterctl and install** (components are published as GitHub Release [v0.1.1](https://github.com/weimantian/cloudnative-cluster-api-provider-cce/releases/tag/v0.1.1); for local development instead build the image and point clusterctl at a local `file://` source as documented in `scripts/deploy-kind.sh`):
   ...
   kubectl get pods -A | grep -E 'capi-|cert-manager|cluster-api-cce'   # all Running
   ```
